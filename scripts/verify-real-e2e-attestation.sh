...
for tool in git python3 timeout; do
  command -v -- "${tool}" >/dev/null 2>&1 || {
    printf 'real-e2e verifier: required tool missing: %s\n' "${tool}" >&2
    exit 1
  }
done
...
def reject_constant(value: str) -> None:
    raise SystemExit(f"real-e2e attestation contains non-finite JSON value: {value}")
...
def unique_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise SystemExit(f"real-e2e attestation contains duplicate JSON key: {key}")
        result[key] = value
    return result
...
if not isinstance(no_follow, int) or isinstance(no_follow, bool) or no_follow <= 0:
    raise SystemExit("real-e2e attestation cannot be opened safely on this platform")
fd = None
primary_error = None
try:
    ...
    if (
        ...
        raise SystemExit("real-e2e attestation changed while reading")
    data = json.loads(
        raw.decode("utf-8"),
    ...
    if fd is not None:
        try:
            ...
            if primary_error is not None:
                primary_error.add_note("real-e2e attestation descriptor cleanup failed")
            else:
                raise SystemExit("real-e2e attestation descriptor cleanup failed") from cleanup_error
if not isinstance(data, dict):
    raise SystemExit("real-e2e attestation must be a JSON object")
...
if (
    ...
    raise SystemExit("real-e2e attestation schema version is unsupported; rerun make real-e2e-acceptance")
if set(data) - {"schema_version", "git_head", "created_at", "expires_at", "matrix", "source"}:
    raise SystemExit("real-e2e attestation contains unexpected fields")
try:
    created = datetime.fromisoformat(data["created_at"].replace("Z", "+00:00"))
    expires = datetime.fromisoformat(data["expires_at"].replace("Z", "+00:00"))
except (KeyError, TypeError, ValueError) as exc:
    raise SystemExit("real-e2e attestation timestamp or expiry is invalid") from exc
if created.tzinfo is None or expires.tzinfo is None or expires - created != ATTESTATION_TTL:
    raise SystemExit("real-e2e attestation expiry contract is invalid")
...
if not isinstance(attested_source, list) or not attested_source:
    raise SystemExit("real-e2e attestation source snapshot is missing")
try:
    ...
    raise SystemExit("real-e2e current source tree is not attestable") from exc
canonical = lambda value: json.dumps(value, sort_keys=True, separators=(",", ":"))
if canonical(attested_source) != canonical(current_source):
    raise SystemExit("real-e2e attestation source changed; rerun acceptance")
PY
