...
class NextVersionTest(unittest.TestCase):
    ...
    def test_float_add_commits_is_rejected(self) -> None:
        code, stderr = run_version_fail_stdout_stderr("--base", "0.1.20", "--add-commits", "1.0")
        self.assertEqual(code, 2)
        self.assertIn("error", stderr.lower())
    ...
    def test_read_current_version_directory_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with self.assertRaises(next_version.UserInputError):
    ...
    def test_read_current_version_non_string_version_value(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir, "pyproject.toml")
            path.write_text("[project]\nversion = 123\n", encoding="utf-8")
            with self.assertRaises(next_version.UserInputError):
    ...
    def test_assert_non_negative_int_rejects_bool(self) -> None:
        ...
        with self.assertRaises(next_version.UserInputError):
            next_version._assert_non_negative_int("minor", None)  # type: ignore[arg-type]
        with self.assertRaises(next_version.UserInputError):
            next_version._assert_non_negative_int("minor", 1.5)  # type: ignore[arg-type]
    ...
    def test_apply_breaking_change_rejects_non_int_values(self) -> None:
        ...
        with self.assertRaises(next_version.UserInputError):
            next_version.apply_breaking_change(1.0, 2, 3)  # type: ignore[arg-type]
        with self.assertRaises(next_version.UserInputError):
    ...
    def test_commits_since_ref_rejects_empty_ref(self) -> None:
        ...
        with self.assertRaises(next_version.UserInputError):
            next_version.commits_since_ref(1.0)  # type: ignore[arg-type]
