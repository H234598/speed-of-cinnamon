...
class PostProcessorTest(unittest.TestCase):
    ...
    def test_openai_compatible_backend_fails_when_flex_is_rejected_without_opt_in(self) -> None:
        ...
        with mock.patch("speed_of_cinnamon.postprocessor._open_http_request", side_effect=fake_urlopen):
            with self.assertRaisesRegex(PostProcessError, "OpenAI-compatible request failed \\(400\\)"):
                post_process_text(
                    "hello cinnamon",
                    "en",
                    backend="openai-compatible",
    ...
    def test_openai_compatible_backend_flex_fallback_uses_raw_error_detail(self) -> None:
        requests = []
        ...
        with (
            mock.patch("speed_of_cinnamon.postprocessor._open_http_request", side_effect=fake_urlopen),
            mock.patch("speed_of_cinnamon.postprocessor._sanitize_remote_error_detail", return_value="[redacted remote error]"),
        ):
            result = post_process_text(
                "hello cinnamon",
                "en",
